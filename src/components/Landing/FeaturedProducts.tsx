  ...
  if (isLoading) {
    return (
      <section className="py-32">
        <div className="mx-auto max-w-7xl px-6 text-center text-text-tertiary font-mono text-xs">
          Cargando...
        </div>
      </section>
    )
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
        <div ref={titleRef} className="flex items-end justify-between mb-16">
          <div>
            ...
            <h2 className="font-display text-5xl sm:text-6xl mt-4 text-text leading-[1.1]">
              Selección
              <br />
              <span className="italic text-accent-text">Premium</span>
            </h2>
          </div>
          <Link
            to="/products"
        ...
        >
          {featured.map((product) => (
            <Card key={product.id} className="flex flex-col p-6 border-0 group">
              <div className="mb-6 flex items-start justify-between">
                <Badge>{categoryLabels[product.category] ?? product.category}</Badge>
                <span className="font-mono text-[10px] text-text-tertiary">
                  ${(product.price / 1000).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
                </span>
              </div>
